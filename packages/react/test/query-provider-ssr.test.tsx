...
describe("SSR Prefetch + Hydration 통합 테스트", () => {
	...
	describe("기본 SSR Prefetch + Hydration", () => {
		...
		it("params 있는 쿼리: SSR prefetch → hydrate → useQuery", async () => {
			...
			// SSR: prefetch
			const dehydratedState = await ssrPrefetch([[testQueries.user, { id: 1 }]]);
			expect(fetchMock).toHaveBeenCalledWith("/api/user/1?id=1", expect.any(Object));

			// Client: hydrate + useQuery
			const queryClient = new QueryClient();
			const { result } = renderHook(() => useQuery(testQueries.user, { params: { id: 1 } }), {
				wrapper: ({ children }) => (
					<QueryClientProvider client={queryClient}>
						<HydrationBoundary state={dehydratedState}>{children}</HydrationBoundary>
					</QueryClientProvider>
	...
	describe("새로운 간단한 API 테스트", () => {
		...
		it("파라미터 있는 쿼리: [query, params] 형태 사용", async () => {
			...
			// 데이터가 올바르게 캐시되었는지 확인
			expect(Object.keys(dehydratedState)).toContain(JSON.stringify(["user", 1]));
		});
		...
		it("혼합 사용: 파라미터 있는 것과 없는 것을 함께 사용", async () => {
			...
			expect(fetchMock).toHaveBeenNthCalledWith(2, "/api/user/1?id=1", expect.any(Object));
			expect(fetchMock).toHaveBeenNthCalledWith(3, "/api/posts", expect.any(Object));
		...
		it("깔끔한 API: 불필요한 undefined 없이 사용", async () => {
			fetchMock.mockResolvedValueOnce(mockFetchResponse(ME_DATA)).mockResolvedValueOnce(mockFetchResponse(POSTS_DATA));

			// 모든 파라미터 없는 쿼리를 깔끔하게 표현
			const dehydratedState = await ssrPrefetch([[testQueries.me], [testQueries.posts]]);
	...
	describe("여러 쿼리 동시 Prefetch", () => {
		it("여러 쿼리를 동시에 prefetch하고 개별적으로 hydrate됨", async () => {
			...
			// 여러 쿼리 동시 prefetch
			const dehydratedState = await ssrPrefetch([[testQueries.me], [testQueries.user, { id: 1 }], [testQueries.posts]]);
			...
			const { result: meResult } = renderHook(() => useQuery(testQueries.me, {}), {
				wrapper: ({ children }) => (
					<QueryClientProvider client={queryClient}>
						<HydrationBoundary state={dehydratedState}>{children}</HydrationBoundary>
					</QueryClientProvider>
			...
			const { result: userResult } = renderHook(() => useQuery(testQueries.user, { params: { id: 1 } }), {
				wrapper: ({ children }) => (
					<QueryClientProvider client={queryClient}>
						<HydrationBoundary state={dehydratedState}>{children}</HydrationBoundary>
					</QueryClientProvider>
	...
	describe("에러 처리 및 고급 기능", () => {
		...
		it("retry 기능이 실제로 동작함", async () => {
			...
			fetchMock
				...
				.mockResolvedValueOnce(mockFetchResponse({ data: "success after retry" }));

			const dehydratedState = await ssrPrefetch([[queryWithRetry.unstable]]);

			// 재시도 후 성공한 데이터가 포함되어야 함
			expect(Object.keys(dehydratedState)).toContain(JSON.stringify(["unstable"]));
		...
		it("timeout 기능이 실제로 동작함", async () => {
			...
			expect(Object.keys(dehydratedState)).not.toContain(JSON.stringify(["slow"]));
