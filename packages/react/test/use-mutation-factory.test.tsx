...
const userMutations = createMutationFactory({
	...
	updateUser: {
		...
		responseSchema: userResponseSchema,
		invalidateQueries: (data, variables) => [["users"], ["users", variables.id]],
	},
...
describe("useMutation Factory-based 사용법", () => {
	...
	describe("URL + Method 방식", () => {
		it("POST 요청으로 사용자 생성 성공", async () => {
			...
			vi.spyOn(client.getFetcher(), "request").mockResolvedValueOnce(mockResponse(responseData));

			const { result } = renderHook(() => useMutation(userMutations.createUser), { wrapper: createWrapper(client) });
		...
		it("동적 URL로 사용자 수정 성공", async () => {
			...
			const { result } = renderHook(() => useMutation(userMutations.updateUser), { wrapper: createWrapper(client) });
		...
		it("DELETE 요청으로 사용자 삭제 성공", async () => {
			vi.spyOn(client.getFetcher(), "request").mockResolvedValueOnce(mockResponse({ success: true }));

			const { result } = renderHook(() => useMutation(userMutations.deleteUser), { wrapper: createWrapper(client) });
	...
	describe("Custom Function 방식", () => {
		it("복잡한 다단계 mutation 성공", async () => {
			...
			const { result } = renderHook(() => useMutation(userMutations.createUserWithProfile), {
		...
		it("배치 업데이트에서 일부 실패 처리", async () => {
			...
			const { result } = renderHook(() => useMutation(userMutations.batchUpdateUsers), {
				wrapper: createWrapper(client),
			});
	...
	describe("콜백 및 상태 관리", () => {
		it("onSuccess 콜백 실행", async () => {
			...
			const { result } = renderHook(() => useMutation(userMutations.deleteUser, { onSuccess: onSuccessSpy }), {
				wrapper: createWrapper(client),
			});
	...
	describe("에러 처리", () => {
		it("네트워크 에러 처리", async () => {
			vi.spyOn(client.getFetcher(), "request").mockRejectedValueOnce(new Error("Network error"));
