...
import { createQueryFactory, QueryClient, z } from "../src/index";
...
const userQueries = createQueryFactory({
	...
	// 파라미터가 선택적인 쿼리
	getUserProfile: {
		cacheKey: (params?: { id?: number }) => ["user", "profile", params?.id] as const,
		url: (params?: { id?: number }) => `/api/user/${params?.id || "me"}/profile`,
...
describe("useQuery Factory-based 사용법", () => {
	...
	describe("파라미터가 필수인 쿼리", () => {
		it("정상적인 파라미터 전달 시 성공", async () => {
			vi.spyOn(client.getFetcher(), "get").mockResolvedValueOnce(mockResponse({ id: 1, name: "Alice" }));
		...
		it("스키마 검증 실패 시 에러 반환", async () => {
			vi.spyOn(client.getFetcher(), "get").mockResolvedValueOnce(mockResponse({ id: "invalid", name: "Alice" }));
	...
	describe("파라미터가 선택적인 쿼리", () => {
		it("파라미터 없이 호출 시 기본값 사용", async () => {
			...
			const { result } = renderHook(() => useQuery(userQueries.getUserProfile, {}), {
				wrapper: createWrapper(client),
			});
			...
			expect(client.getFetcher().get).toHaveBeenCalledWith("/api/user/me/profile", expect.any(Object));
		});
		...
		it("파라미터와 함께 호출 시 해당 파라미터 사용", async () => {
			...
			const { result } = renderHook(() => useQuery(userQueries.getUserProfile, { params: { id: 42 } }), {
				wrapper: createWrapper(client),
			});
			...
			expect(client.getFetcher().get).toHaveBeenCalledWith("/api/user/42/profile", expect.any(Object));
		});
	});
	...
	describe("파라미터가 없는 쿼리", () => {
		it("리스트 쿼리 정상 동작", async () => {
			...
			const { result } = renderHook(() => useQuery(userQueries.getUserList, {}), {
				wrapper: createWrapper(client),
			});
			...
			expect(client.getFetcher().get).toHaveBeenCalledWith("/api/users", expect.any(Object));
		});
	});
	...
	describe("조건부 enabled 쿼리", () => {
		it("짧은 검색어 시 fetch 하지 않음", () => {
			...
			const { result } = renderHook(() => useQuery(userQueries.searchUsers, { params: "ab" }), {
				wrapper: createWrapper(client),
			});
		...
		it("긴 검색어 시 fetch 실행", async () => {
			vi.spyOn(client.getFetcher(), "get").mockResolvedValueOnce(mockResponse([{ id: 1, name: "Alice Anderson" }]));

			const { result } = renderHook(() => useQuery(userQueries.searchUsers, { params: "alice" }), {
				wrapper: createWrapper(client),
			});
			...
			expect(client.getFetcher().get).toHaveBeenCalledWith("/api/users/search?q=alice", expect.any(Object));
		});
	});
	...
	describe("Factory 옵션 오버라이드", () => {
		it("select 함수가 있는 Factory 쿼리", async () => {
			...
			const { result } = renderHook(() => useQuery(userQueries.getUserWithTransform, { params: 1 }), {
				wrapper: createWrapper(client),
			});
	...
	describe("queryFn 기반 쿼리", () => {
		it("queryFn으로 여러 API 호출 및 데이터 조합", async () => {
			...
			const { result } = renderHook(() => useQuery(userQueries.getUserWithPosts, { params: 1 }), {
				wrapper: createWrapper(client),
			});
		...
		it("queryFn에서 조건부 로직 처리", async () => {
			...
			// 조건부로 추가 API 호출 확인
			expect(client.getFetcher().get).toHaveBeenCalledTimes(2);
			expect(client.getFetcher().get).toHaveBeenCalledWith("/api/analytics/1?start=2024-01-01&end=2024-01-31");
			expect(client.getFetcher().get).toHaveBeenCalledWith("/api/analytics/1/additional");
		});
	...
	describe("런타임 검증", () => {
		...
		it("createQueryFactory: url과 queryFn 둘 다 없으면 에러", () => {
			...
			}).toThrow("Invalid QueryConfig for 'invalid': QueryConfig must have either 'queryFn' or 'url'");
		});
	});
