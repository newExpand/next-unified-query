...
describe("useQuery", () => {
	...
	describe("Options-based 사용법", () => {
		it("기본 GET 요청 성공", async () => {
			vi.spyOn(client.getFetcher(), "get").mockResolvedValueOnce(mockResponse({ name: "Alice" }));
		...
		it("에러 처리", async () => {
			vi.spyOn(client.getFetcher(), "get").mockRejectedValueOnce(new Error("Network error"));
		...
		it("select 옵션으로 데이터 변환", async () => {
			vi.spyOn(client.getFetcher(), "get").mockResolvedValueOnce(mockResponse({ name: "Alice", age: 20 }));
		...
		it("placeholderData: 함수로 이전 데이터 유지", async () => {
			vi.spyOn(client.getFetcher(), "get").mockResolvedValueOnce(mockResponse({ name: "A" }));
			...
			vi.spyOn(client.getFetcher(), "get").mockResolvedValueOnce(mockResponse({ name: "B" }));
			rerender({ id: 2 });
	...
	describe("Factory-based 사용법", () => {
		it("파라미터가 있는 쿼리 팩토리 사용", async () => {
			vi.spyOn(client.getFetcher(), "get").mockResolvedValueOnce(mockResponse({ name: "Alice", id: 1 }));

			const { result } = renderHook(() => useQuery(userQueries.getUser, { params: 1 }), {
				wrapper: createWrapper(client),
			});
			...
			expect(client.getFetcher().get).toHaveBeenCalledWith("/api/user/1", expect.any(Object));
		});

		it("조건부 enabled가 있는 쿼리 팩토리", async () => {
			// 짧은 검색어 (enabled=false)
			const fetcherSpy1 = vi.spyOn(client.getFetcher(), "get");
			const { result: r1 } = renderHook(() => useQuery(userQueries.searchUsers, { params: "ab" }), {
				wrapper: createWrapper(client),
			});
			...
			const { result: r2 } = renderHook(() => useQuery(userQueries.searchUsers, { params: "alice" }), {
				wrapper: createWrapper(client),
			});
			...
			expect(fetcherSpy1).toHaveBeenCalledWith("/api/users/search?q=alice", expect.any(Object));
		});
	...
	describe("캐시 및 상태 관리", () => {
		it("쿼리키별 캐시 동작", async () => {
			...
			const { result: r1 } = renderHook(() => useQuery({ cacheKey: ["user", 1], url: "/api/user/1" }), {
				wrapper: createWrapper(client),
			});

			const { result: r2 } = renderHook(() => useQuery({ cacheKey: ["user", 2], url: "/api/user/2" }), {
				wrapper: createWrapper(client),
			});
		...
		it("staleTime: fresh 상태에서는 재마운트 시 fetch 생략", async () => {
			vi.useRealTimers();
			vi.spyOn(client.getFetcher(), "get").mockResolvedValue(mockResponse({ name: "A" }));
			...
			await waitFor(() => expect(client.getFetcher().get).toHaveBeenCalledTimes(1));
			...
			await waitFor(() => expect(client.getFetcher().get).toHaveBeenCalledTimes(2));
		}, 10000);
	});
	...
	describe("런타임 검증 - Options", () => {
		it("url과 queryFn 둘 다 제공하면 에러", () => {
			...
			}).toThrow("QueryConfig cannot have both 'queryFn' and 'url' at the same time");
		});
	...
	describe("다중 캐시키 처리", () => {
		it("인자값으로 받는 케이스: 복합 캐시키 처리", async () => {
			...
			expect(client.getFetcher().get).toHaveBeenCalledWith("/api/user/1/posts?page=1", expect.any(Object));
		...
		it("인자값으로 받는 케이스: 검색 쿼리와 필터 조합", async () => {
			...
			const { result } = renderHook(
				() =>
					useQuery({
						cacheKey: ["users", "search", "al", "department", "engineering", "page", 1],
			...
			const cached = client.get(["users", "search", "al", "department", "engineering", "page", 1]);
		...
		it("캐시키 변경 시 별도 캐시 생성", async () => {
			vi.spyOn(client.getFetcher(), "get")
				.mockResolvedValueOnce(mockResponse({ page: 1, data: ["item1", "item2"] }))
				.mockResolvedValueOnce(mockResponse({ page: 2, data: ["item3", "item4"] }));
		...
		it("동일한 객체 구조의 캐시키는 같은 캐시 사용", async () => {
			vi.spyOn(client.getFetcher(), "get").mockResolvedValueOnce(mockResponse({ shared: "data" }));
	...
	describe("Prefetch 및 Hydration", () => {
		it("prefetch 후 hydrate하면 useQuery에서 fetcher 호출 없이 캐시 사용", async () => {
			...
			const { result: r2 } = renderHook(() => useQuery({ cacheKey: keyB, url: "/api/user/2", staleTime: 10000 }), {
				wrapper: createWrapper(client),
			});
	...
	describe("Select 함수 최적화 (selectDeps)", () => {
		describe("기본 동작", () => {
			it("selectDeps 없이 인라인 select 함수는 매번 재실행됨", async () => {
				vi.spyOn(client.getFetcher(), "get").mockResolvedValueOnce(mockResponse({ name: "Alice", age: 25 }));
					...
					({ categories }: { categories: string[] }) => {
						return useQuery({
							...
							select: (data: any) => {
								...
								return {
									...data,
									filtered: data.items.filter((item: string) => categories.some((cat) => item.includes(cat))),
								};
							},
		...
		describe("성능 최적화", () => {
			it("select 함수 호출 횟수 추적", async () => {
				vi.spyOn(client.getFetcher(), "get").mockResolvedValueOnce(mockResponse({ count: 10 }));
