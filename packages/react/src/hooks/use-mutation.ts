...
interface UrlBasedUseMutationOptions<
  ...
  RequestSchema extends ZodType = ZodType,
  ResponseSchema extends ZodType = ZodType
> extends BaseUseMutationOptions<
    TData,
    TError,
    TVariables,
    TContext,
...
>(
  ...
  const isFactoryConfig =
    "url" in configOrOptions || "mutationFn" in configOrOptions;

  if (isFactoryConfig && overrideOptions) {
    // Factory 기반 + override options
    const factoryConfig = configOrOptions as MutationConfig<
      TVariables,
      TData,
      TError,
      TContext
...
>(
  ...
  const getMutationFn = useCallback(() => {
    ...
    return async (variables: TVariables, fetcher: NextTypeFetch) => {
      const urlBasedOptions = options as UrlBasedUseMutationOptions<
        TData,
        TError,
        TVariables,
        TContext
      >;
      const url = isFunction(urlBasedOptions.url)
        ? urlBasedOptions.url(variables)
        : urlBasedOptions.url;
