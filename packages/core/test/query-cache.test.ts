...
import {
  ...
  QueryState,
  serializeQueryKey,
} from "../src/query/cache/query-cache";
...
describe("QueryCache", () => {
  ...
  describe("serializeQueryKey 유틸리티", () => {
    it("배열 키를 JSON 문자열로 변환", () => {
      expect(serializeQueryKey(["user", 1, "profile"])).toBe(
        JSON.stringify(["user", 1, "profile"])
      );
    });
  ...
  describe("엣지 케이스 및 에러 처리", () => {
    ...
    it("존재하지 않는 키에 대한 subscribe/unsubscribe 안전 처리", () => {
      ...
      // 리스너도 마찬가지
      expect(() => {
        const unsubscribe = queryCache.subscribeListener(
          nonExistentKey,
          () => {}
        );
        unsubscribe();
