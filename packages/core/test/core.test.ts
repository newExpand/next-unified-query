...
describe("next-type-fetch: 코어 기능", () => {
	...
	it("URL 조합 테스트 (명시적 경로 보존)", async () => {
		...
		expect(mockFetch.mock.calls[3][0]).toBe("https://api.example.com/v1/../parent/path");
		...
		expect(mockFetch.mock.calls[5][0]).toBe("https://api.example.com/v1/nested/../parent-resource");
	});
});
