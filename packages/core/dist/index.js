...
function appendQueryParams(url, params) {
  ...
  const urlParts = esToolkit.compact([
    path,
...
function serializeQueryKey(key) {
  return compat.isArray(key) ? JSON.stringify(key) : compat.isString(key) ? key : String(key);
}
...
function replaceEqualDeep(prev, next) {
  ...
  if (compat.isEqual(prev, next)) {
    return prev;
  }
  if (prev == null || next == null) {
    return next;
  }
  if (compat.isArray(prev) && compat.isArray(next)) {
  ...
  if (compat.isPlainObject(prev) && compat.isPlainObject(next)) {
    const prevObj = prev;
    const nextObj = next;
    const prevKeys = compat.keys(prevObj);
    const nextKeys = compat.keys(nextObj);
...
var FetchManager = class {
  ...
  buildFetchConfig(options2) {
    let config = compat.merge({}, options2.fetchConfig ?? {});
    if (predicate.isNotNil(options2.params)) {
      config = compat.merge(config, { params: options2.params });
    }
    if (predicate.isNotNil(options2.schema)) {
      config = compat.merge(config, { schema: options2.schema });
    }
    return config;
  }
  /**
...
var OptionsManager = class {
  ...
  createOptionsHash(options2) {
    const hashableOptions = compat.pick(options2, [
      "key",
      "url",
      "params",
      "enabled",
...
var PlaceholderManager = class {
  ...
  computePlaceholderData(options2) {
    ...
    if (!predicate.isFunction(placeholderData)) {
  ...
  isDifferentQueryKey(keyArray, currentKey) {
    return !compat.isEqual(keyArray, currentKey);
  }
  /**
   * 유효한 데이터가 있는지 확인
   */
  hasValidData(state) {
    return state && !compat.isNil(state.data);
  }
  /**
   * 더 최근 데이터인지 확인
   */
  ...
  hasValidPlaceholderData(placeholderData) {
    return !compat.isNil(placeholderData);
  }
  /**
   * PlaceholderState 설정
   */
...
var ResultComputer = class {
  ...
  isSuccessState(cached) {
    return !cached.isLoading && !cached.error && !compat.isNil(cached.data);
  }
  /**
   * select 함수 적용
   */
  applySelect(data, options2) {
    if (compat.isNil(data) || !options2.select) return data;
    try {
...
var QueryObserver = class {
  ...
  hasNoTrackedProperties(trackedProps) {
    return compat.isEmpty(trackedProps);
  }
  ...
  const results = await Promise.allSettled(
    queries.map(async (queryItem) => {
      try {
        ...
        const mergedQuery = {
          ...query,
          fetchConfig: compat.merge({}, globalFetchConfig, query.fetchConfig || {})
        };
...
exports.createMutationFactory = createMutationFactory;
exports.createQueryClientWithInterceptors = createQueryClientWithInterceptors;
exports.createQueryFactory = createQueryFactory;
exports.defaultInstance = fetch_default;
exports.del = del;
...
exports.head = head;
exports.interceptorTypes = interceptorTypes;
exports.interceptors = interceptors;
exports.isFetchError = isFetchError;
exports.ntFetch = ntFetch;
...
exports.setDefaultQueryClientOptions = setDefaultQueryClientOptions;
exports.ssrPrefetch = ssrPrefetch;
exports.unwrap = unwrap;
exports.validateMutationConfig = validateMutationConfig;
exports.validateQueryConfig = validateQueryConfig;
