 ...
 */
function setupAbortSignal(signal: AbortSignal | undefined, onAbort: () => void): void {
	if (!signal) return;
 ...
 */
function createRequestInit(requestConfig: RequestConfig, abortController: AbortController): RequestInit {
	const {
		method = "GET",
		headers = {},
		cache,
 ...
 * 재시도 가능 여부를 확인합니다
 */
function canRetry(retryCount: number, maxRetries: number, isCanceled: boolean): boolean {
 ...
 */
function shouldRetryForHttpStatus(status: number, retryStatusCodes: number[]): boolean {
 ...
 */
function shouldAuthRetryForStatus(status: number, authRetryOption: AuthRetryOption): boolean {
