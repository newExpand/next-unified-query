...
export class PlaceholderManager<T = unknown> {
	 ...
	 */
	private findPreviousQuery<T>(options: QueryObserverOptions<T>): QueryState<T> | undefined {
	 ...
	 * 배열 키인지 확인
	 */
	private isArrayKey<T>(keyArray: any, options: QueryObserverOptions<T>): boolean {
	 ...
	 */
	private isSameQueryType(keyArray: any[], currentKey: readonly unknown[]): boolean {
	 ...
	 */
	private isDifferentQueryKey(keyArray: any[], currentKey: readonly unknown[]): boolean {
