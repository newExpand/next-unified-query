...
interface BaseQueryConfig<Params = void, Schema extends ZodType = ZodType> {
	...
	schema?: Schema;
	placeholderData?: any | ((prev?: any, prevQuery?: import("../cache/query-cache").QueryState<any>) => any);
 ...
 */
interface FunctionBasedQueryConfig<Params = void, Schema extends ZodType = ZodType>
 ...
 */
export type ExtractQueryData<T> = T extends QueryConfig<any, infer S> ? InferIfZodSchema<S, any> : any;
...
export function createQueryFactory<T extends QueryFactoryInput>(defs: T): T {
	...
	Object.entries(defs).forEach(([key, config]) => {
		try {
			...
			throw new Error(`Invalid QueryConfig for '${key}': ${(error as Error).message}`);
		}
	});
