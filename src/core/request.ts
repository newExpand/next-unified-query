import { z } from "zod";
import type { AxiosLikeResponse, CancelablePromise, FetchConfig, RequestConfig } from "../types/index.js";
import { ContentType, FetchError, ResponseType } from "../types/index.js";
import { appendQueryParams, combineURLs, createTimeoutPromise, stringifyData } from "../utils/index.js";
...
interface InterceptorsType {
	...
	error: {
		run: (error: FetchError) => Promise<AxiosLikeResponse<unknown> | FetchError>;
	};
}
...
function prepareRequestBody(
	...
	switch (true) {
		// JSON 컨텐츠 타입
		case contentTypeStr === ContentType.JSON || contentTypeStr.includes("application/json"):
			return {
		...
		// URL 인코딩된 폼 데이터
		case contentTypeStr === ContentType.FORM || contentTypeStr.includes("application/x-www-form-urlencoded"): {
			let body: BodyInit;
		...
		// XML 컨텐츠 타입
		case contentTypeStr === ContentType.XML || contentTypeStr.includes("application/xml"):
			return {
		...
		// HTML 컨텐츠 타입
		case contentTypeStr === ContentType.HTML || contentTypeStr.includes("text/html"):
			return {
				body: typeof data === "string" ? data : String(data),
		...
		// 일반 텍스트
		case contentTypeStr === ContentType.TEXT || contentTypeStr.includes("text/plain"):
			return {
				body: typeof data === "string" ? data : String(data),
		...
		// 바이너리 데이터
		case contentTypeStr === ContentType.BLOB || contentTypeStr.includes("application/octet-stream"): {
			const body =
				data instanceof Blob || data instanceof ArrayBuffer ? data : typeof data === "string" ? data : String(data);
...
export function createRequestFunction(defaultConfig: FetchConfig, interceptors: InterceptorsType) {
	 ...
	 * 기본 요청 함수
	 */
	function request<T>(config: RequestConfig): CancelablePromise<AxiosLikeResponse<T>> {
		// 취소 상태 관리
		...
		let retryBackoff: (retryCount: number) => number = (count) => Math.min(1000 * 2 ** (count - 1), 10000);
		...
		async function performRequest(): Promise<AxiosLikeResponse<T>> {
			try {
				...
				// 응답 인터셉터 실행
				const processedResponse = await interceptors.response.run(axiosLikeResponse);
				...
				if (retryCount < maxRetries && !isCanceled) {
					...
					if (
						...
						error.response &&
						(retryStatusCodes.length === 0 || retryStatusCodes.includes(error.response.status))
					) {
						retryCount++;
