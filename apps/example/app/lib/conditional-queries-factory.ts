...
export const conditionalQueries = createQueryFactory({
  ...
  productModels: {
    ...
    url: (params: [string, string]) => `/api/models?category=${params[0]}&brand=${params[1]}`
  },
  productSpecs: {
  ...
  // 검색 관련 쿼리들
  searchRealTime: {
    cacheKey: (query: string, category: string) => ["search", query, category],
    url: (query: string, category: string) => `/api/search?q=${query}&category=${category}`
  },
  searchProducts: {
    cacheKey: (filters: Record<string, string>) => ["products", "search", filters],
  ...
  // 프로젝트 관련 동적 라우팅 쿼리들
  projectData: {
    cacheKey: (projectId: number, view: string) => ["projects", projectId, "data", view],
    url: (projectId: number, view: string) => `/api/projects/${projectId}/data?view=${view}`
  }
});
