...
export default function ConditionalRoutingProjectPage() {
  ...
  // URL 파라미터 기반 동적 쿼리 키
  const { data: projectData, isLoading, error } = useQuery<ProjectData>({
    cacheKey: ["projects", projectId, "data", view],
    queryFn: async () => {
      // optimistic update: 뷰 변경 시 즉시 로딩 상태 표시
      queryClient.setQueryData(["projects", projectId, "data", view], (old: ProjectData | undefined) => ({
        ...old,
        projectId,
        view,
        data: "로딩 중...",
      ...
      const response = await fetch(`/api/projects/${projectId}/data?view=${view}`);
      if (!response.ok) {
        ...
        <div className="bg-white shadow rounded-lg p-6">
          ...
          <div className="mb-6 bg-blue-50 border border-blue-200 rounded-lg p-4">
            ...
            <div className="text-sm text-blue-700">
              <p>프로젝트 ID: <span className="font-mono">{projectId}</span></p>
              <p>뷰: <span className="font-mono">{view}</span></p>
              <p>쿼리 키: <span className="font-mono">["projects", {projectId}, "data", "{view}"]</span></p>
            </div>
          </div>
          ...
          <div className="mb-6">
            ...
            {isLoading ? (
              ...
              <div className="bg-gray-50 border border-gray-200 rounded-lg p-4" data-testid="project-data">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <h4 className="font-medium text-gray-800 mb-2">프로젝트 정보</h4>
                    <div className="space-y-1 text-sm">
                      <p><span className="text-gray-600">ID:</span> {projectData.projectId}</p>
                      <p><span className="text-gray-600">뷰:</span> {projectData.view}</p>
                      <p><span className="text-gray-600">타임스탬프:</span> {new Date(projectData.timestamp).toLocaleString()}</p>
                    </div>
                  </div>
                  <div>
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                <div>
                  ...
                  <span className={`ml-2 font-medium ${
                    ...
                    error ? "text-red-600" : 
                    projectData ? "text-green-600" : "text-gray-500"
                  }`}>
                    {isLoading ? "로딩 중" : 
                     error ? "에러" : 
                     projectData ? "완료" : "대기"}
              ...
              <div className="mt-4 pt-4 border-t">
                <p className="text-gray-600 text-sm">
                  <strong>조건부 쿼리 동작:</strong> 프로젝트 ID와 뷰 파라미터가 모두 있을 때만 쿼리가 실행됩니다.
                  URL 변경 시 새로운 쿼리 키로 데이터를 다시 로드하며, 캐시된 데이터는 5분간 유지됩니다.
                </p>
              </div>
            </div>
          </div>
