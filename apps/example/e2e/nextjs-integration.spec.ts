...
test.describe("Next.js App Router SSR and RSC", () => {
  test("Server Component에서 prefetch한 데이터가 Client Component에서 hydration", async ({ page }) => {
    ...
    const ssrUserName = await page.locator('[data-testid="user-name"]').textContent();
    ...
    const hydratedUserName = await page.locator('[data-testid="user-name"]').textContent();
  ...
  test("Server Component + Client Component 혼합 데이터 페칭", async ({ page }) => {
  ...
  test("generateStaticParams vs dynamic routes", async ({ page }) => {
    // Static params로 생성된 페이지
    await page.goto("/posts/1");
    await page.waitForSelector('[data-testid="static-post"]');
    
    const staticLoadTime = await page.evaluate(() => performance.now());
    
    // Dynamic route 페이지
    await page.goto("/posts/999");
    await page.waitForSelector('[data-testid="dynamic-post"]');
    
    const dynamicLoadTime = await page.evaluate(() => performance.now());
    
    // Static 페이지가 더 빠르게 로드되어야 함
    expect(staticLoadTime).toBeLessThan(dynamicLoadTime);
  });
});

test.describe("Next.js Cache Options", () => {
  test("force-cache 옵션으로 브라우저 캐시 활용", async ({ page }) => {
    let requestCount = 0;
    ...
    // force-cache로 인해 추가 요청이 발생하지 않아야 함
    expect(requestCount).toBe(1);
  ...
  test("revalidate 옵션으로 시간 기반 재검증", async ({ page }) => {
    ...
    const cachedTimestamp = await page.locator('[data-testid="data-timestamp"]').textContent();
    ...
    const revalidatedTimestamp = await page.locator('[data-testid="data-timestamp"]').textContent();
  ...
  test("tags를 사용한 selective revalidation", async ({ page, context }) => {
    ...
    const initialData = await page1.locator('[data-testid="tagged-content"]').textContent();
    ...
    // 데이터가 업데이트되었는지 확인
    const updatedData = await page1.locator('[data-testid="tagged-content"]').textContent();
...
test.describe("App Router Special Files", () => {
  test("loading.js UI와 Suspense 경계", async ({ page }) => {
    ...
    // 2초 후 실제 페이지 표시
    await page.waitForSelector('[data-testid="user-content"]', { timeout: 4000 });
  ...
  test("layout.js 중첩과 상태 유지", async ({ page }) => {
    ...
    const layoutSidebar = await page.locator('[data-testid="layout-sidebar"]').textContent();
    ...
    // 레이아웃은 유지되면서 내용만 변경
    const preservedSidebar = await page.locator('[data-testid="layout-sidebar"]').textContent();
...
test.describe("Next.js Performance Features", () => {
  ...
  test("Dynamic Import와 lazy loading", async ({ page }) => {
    ...
    // 동적 컴포넌트의 쿼리 데이터 확인
    await expect(page.locator('[data-testid="heavy-component-data"]')).toBeVisible();
  });
  ...
  test("App Router의 fetch cache와 revalidate", async ({ page }) => {
    ...
    const firstTimestamp = await page.locator('[data-testid="server-timestamp"]').textContent();
    
    // 페이지 새로고침 - 기본적으로 캐시됨
    await page.reload();
    await page.waitForSelector('[data-testid="cached-content"]');
    const secondTimestamp = await page.locator('[data-testid="server-timestamp"]').textContent();
    ...
    const thirdTimestamp = await page.locator('[data-testid="server-timestamp"]').textContent();
