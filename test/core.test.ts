...
describe("next-type-fetch: 코어 기능", () => {
	...
	it("기본 설정과 요청별 설정 병합", async () => {
		...
		// 헤더가 올바르게 병합되었는지 확인
		expect(requestInit.headers.Authorization).toBe("Bearer request-specific-token");
	...
	it("동시 요청 처리", async () => {
		...
		// 동시에 여러 요청 실행
		const results = await Promise.all([api.get("/users/1"), api.get("/users/2"), api.get("/users/3")]);
	...
	it("상대 경로 URL 조합 테스트", async () => {
		...
		expect(mockFetch.mock.calls[0][0]).toBe("https://api.example.com/v1/absolute/path");

		await api.get("relative/path");
		expect(mockFetch.mock.calls[1][0]).toBe("https://api.example.com/v1/relative/path");

		await api.get("./current/path");
		expect(mockFetch.mock.calls[2][0]).toBe("https://api.example.com/v1/current/path");

		await api.get("../parent/path");
		expect(mockFetch.mock.calls[3][0]).toBe("https://api.example.com/parent/path");
		...
		expect(mockFetch.mock.calls[4][0]).toBe("https://api.example.com/v1/nested/resource");

		await nestedApi.get("../parent-resource");
		expect(mockFetch.mock.calls[5][0]).toBe("https://api.example.com/v1/parent-resource");
	});
});
