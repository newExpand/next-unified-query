...
import { get, post, put, patch, head, options, del, request, ntFetch, interceptors } from "../src/index.js";
...
describe("next-type-fetch: 기본 인스턴스", () => {
	...
	it("OPTIONS 메서드 직접 사용", async () => {
		...
		expect(result.status).toBe(200);
		expect(result.headers.get("access-control-allow-methods")).toBe("GET, POST, PUT, DELETE, PATCH");
	});
