...
import { useMutation } from "../src/query/hooks/use-mutation";
...
import { createMutationFactory } from "../src/query/factories/mutation-factory";
...
describe("useMutation", () => {
              ...
              const userResult = await fetcher.request({
                ...
                headers: {
                  Authorization: `Bearer ${
                    (authResult.data as { token: string }).token
                  }`,
                },
              });
  ...
  describe("Factory-based 사용법", () => {
    ...
    it("Custom Function 팩토리 사용", async () => {
      ...
      const { result } = renderHook(
        () => useMutation(testMutations.complexMutation),
        { wrapper: createWrapper(client) }
      );
  ...
  describe("상태 관리", () => {
    it("isPending 상태 변화", async () => {
      ...
      vi.spyOn(client.getFetcher(), "request").mockReturnValueOnce(
        fetchPromise as any
      );
