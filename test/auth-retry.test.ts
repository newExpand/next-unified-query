...
describe("authRetry 옵션 (401 인증 오류 자동 재시도)", () => {
	...
	it("access token이 만료되어 401 발생 시 refresh token으로 갱신 후 재시도에 성공해야 한다", async () => {
		...
		expect(mockFetch.mock.calls[0][1].headers.Authorization).toBe("Bearer expired-access-token");
	...
	it("statusCodes에 커스텀 상태코드를 지정하면 해당 코드에서만 재시도된다", async () => {
		...
		mockFetchSequence([{ status: 401004 }, { status: 200, body: { ok: true } }]);
		const api = createFetch({
			authRetry: {
				limit: 1,
				statusCodes: [401004],
	...
	it("statusCodes에 여러 상태코드를 지정하면 각 코드에서 재시도된다", async () => {
		...
		for (const code of codes) {
			callCount = 0;
			handlerCount = 0;
			mockFetchSequence([{ status: code }, { status: 200, body: { ok: true } }]);
			const api = createFetch({
				authRetry: {
					limit: 1,
					statusCodes: codes,
