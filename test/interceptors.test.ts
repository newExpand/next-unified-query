...
describe("next-type-fetch: 인터셉터", () => {
  ...
  it("요청 인터셉터 작동 확인", async () => {
    ...
    expect(mockFetch.mock.calls[0][1].headers["X-Test-Header"]).toBe(
      "test-value"
    );
  });
  ...
  it("응답 인터셉터 작동 확인", async () => {
    ...
    // 응답 인터셉터 추가
    api.interceptors.response.use((response: NextTypeResponse<unknown>) => {
      const data = response.data as Record<string, unknown>;
      return {
        ...response,
        data: { ...data, intercepted: true },
      } as NextTypeResponse<unknown>;
    });
  ...
  it("여러 인터셉터 체이닝 테스트", async () => {
    ...
    // 커스텀 인터페이스 정의
    interface ExtendedResponse extends NextTypeResponse<unknown> {
      first?: boolean;
      second?: boolean;
    }
  ...
  it("응답 에러 인터셉터 테스트", async () => {
    ...
    try {
      ...
      // 추가 테스트 실패로 처리
      throw new Error(
        `에러 인터셉터가 에러를 변환하지 못했습니다: ${error.message}`
      );
    }
  });
  ...
  it("비동기 인터셉터 처리 테스트", async () => {
    ...
    // 커스텀 인터페이스 정의
    interface AsyncProcessedResponse extends NextTypeResponse<unknown> {
      asyncProcessed?: boolean;
    }

    // 비동기 응답 인터셉터 추가
    api.interceptors.response.use(
      async (response: NextTypeResponse<unknown>) => {
    ...
    // 요청 헤더 확인
    expect(mockFetch.mock.calls[0][1].headers["X-Async-Header"]).toBe(
      "async-value"
    );
